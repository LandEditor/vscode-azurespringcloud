...
export class AppItem implements ResourceItemBase {
	...
	constructor(
		public readonly parent: AppsItem,
		app: EnhancedApp
	) {
		this.app = app;
		void this.refresh();
	...
	public get description(): string | undefined {
		const state: string | undefined =
			this.app.properties?.provisioningState;
		return state?.toLowerCase() === "succeeded"
			? this.app.activeDeployment?.runtimeVersion
					?.split(/[\s\_]/)
					.join(" ")
			: state;
	}
	...
	public get contextValue(): string {
		const debugging: string =
			this._debuggingEnabled === undefined
				? "unknown"
				: this._debuggingEnabled
				? "enabled"
				: "disabled";
		const tier: string = this.app.service.isEnterpriseTier()
			? "enterprise"
			: this.app.service.isConsumptionTier()
			? "consumption"
			: "other";
			...
			await ext.state.runWithTemporaryDescription(
				...
				async () => {
					...
					const deployment: EnhancedDeployment | undefined =
						await this.app.getActiveDeployment();
	...
	private async reload(): Promise<void> {
		this._children = (async () => {
			...
			return [
				...
				this._scaleSettingsItem,
				new AppJvmOptionsItem(this),
			];
		})();
	}
}
