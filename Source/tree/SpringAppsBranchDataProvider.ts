...
export class SpringAppsBranchDataProvider
	extends vscode.Disposable
	implements AzureResourceBranchDataProvider<ResourceItemBase>
{
	private readonly onDidChangeTreeDataEmitter = new vscode.EventEmitter<
		ResourceItemBase | undefined
	>();
	...
	async getResourceItem(element: AzureResource): Promise<ResourceItemBase> {
		const resourceItem = await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				const apps: EnhancedService = new EnhancedService(
					client,
					element.subscription,
					element
				);
