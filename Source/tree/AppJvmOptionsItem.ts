	...
	public get options(): Promise<string[]> {
		return (async () => {
			const deployment: EnhancedDeployment | undefined =
				await this.parent.app.getActiveDeployment();
			const enterpriseOptionsStr: string | undefined =
				deployment?.properties?.deploymentSettings?.environmentVariables
					?.JAVA_OPTS;
			const source: JarUploadedUserSourceInfo = <
				JarUploadedUserSourceInfo
			>deployment?.properties?.source;
