...
export class EnhancedApp {
	...
	public async getStatus(): Promise<string> {
		const activeDeployment: EnhancedDeployment | undefined =
			await this.getActiveDeployment();
		let _status: string = (
			(activeDeployment?.properties?.status ||
				activeDeployment?.properties?.provisioningState ||
				this.properties?.provisioningState) ??
			"Unknown"
		).toLowerCase();
	...
	public async refresh(): Promise<EnhancedApp> {
		this._remote = await this.client.apps.get(
			this.service.resourceGroup,
			this.service.name,
			this.name
		);
	...
	public async getActiveDeployment(
		force: boolean = false
	): Promise<EnhancedDeployment | undefined> {
		if (force || !this._activeDeployment) {
			this._activeDeployment = (await this.getDeployments()).find(
				(d) => d.properties?.active
			);
		}
		return this._activeDeployment;
	}
	...
	public async setActiveDeployment(deploymentName: string): Promise<void> {
		...
		this._remote = await this.client.apps.beginSetActiveDeploymentsAndWait(
			this.service.resourceGroup,
			this.service.name,
			this.name,
			{
	...
	public async createDeployment(
		...
		if (this.service.isEnterpriseTier()) {
			...
			source = {
				...
				runtimeVersion: runtime ?? EnhancedApp.DEFAULT_RUNTIME,
			};
		}
			...
			await this.client.deployments.beginCreateOrUpdateAndWait(
				...
				name,
				{
					properties: {
						...
						deploymentSettings: {
							resourceRequests: {
								memory: this.service.isConsumptionTier()
									? "2Gi"
									: "1Gi",
								cpu: "1",
							},
	...
	public async setPublic(isPublic: boolean): Promise<void> {
		...
		this._remote = await this.client.apps.beginCreateOrUpdateAndWait(
			this.service.resourceGroup,
			this.service.name,
			this.name,
			{
	...
	public async enqueueBuild(
		...
		while (status !== "Succeeded") {
			...
			const result: BuildResult =
				await this.client.buildServiceOperations.getBuildResult(
					this.service.resourceGroup,
					this.service.name,
					EnhancedApp.DEFAULT_TANZU_COMPONENT_NAME,
					this.name,
