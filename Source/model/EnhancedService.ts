...
export class EnhancedService {
	...
	public async loadDevTools(): Promise<DevToolPortalResource | undefined> {
		for await (const portal of this.client.devToolPortals.list(
			this.resourceGroup,
			this.name
		)) {
	...
	public async enableLiveView(): Promise<void> {
		this._devToolsPortal =
			this.client.devToolPortals.beginCreateOrUpdateAndWait(
				this.resourceGroup,
				this.name,
				"default",
				{
	...
	public async refresh(): Promise<EnhancedService> {
		const remote: ServiceResource = await this.client.services.get(
			this.resourceGroup,
			this.name
		);
