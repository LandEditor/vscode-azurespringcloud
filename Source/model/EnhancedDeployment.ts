...
export class EnhancedDeployment {
	...
	get runtimeVersion(): Promise<string | undefined> {
		return this.properties
			.then((p) => p?.source as JarUploadedUserSourceInfo)
			.then((s) => s.runtimeVersion);
	}
	...
	public async refresh(): Promise<EnhancedDeployment> {
		this._remote = this.client.deployments.get(
	...
	public async enableDebugging(port = 5005): Promise<RemoteDebugging> {
		ext.outputChannel.appendLog(
