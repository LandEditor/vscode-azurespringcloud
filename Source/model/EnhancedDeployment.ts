...
export class EnhancedDeployment {
	...
	get runtimeVersion(): string | undefined {
		return (this.properties?.source as JarUploadedUserSourceInfo)
			?.runtimeVersion;
	}
	...
	public get latestInstance(): EnhancedInstance {
		// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		return this.instances.reduce((prev, current) =>
	...
	public async refresh(): Promise<EnhancedDeployment> {
		this._remote = await this.client.deployments.get(
	...
	public async updateArtifactPath(
		...
		if (this.app.service.isEnterpriseTier()) {
			properties = {
				source: {
					type: "BuildResult",
					buildResultId: relativePathOrBuildId,
				},
			};
		} else {
			properties = {
		...
		this._remote = await this.client.deployments.beginUpdateAndWait(
			this.app.service.resourceGroup,
			this.app.service.name,
			this.app.name,
	...
	public async updateScaleSettings(settings: IScaleSettings): Promise<void> {
		...
		if (this.app.service.isConsumptionTier()) {
			...
			resource.properties!.deploymentSettings!.scale = {
				minReplicas:
					this._remote.properties?.deploymentSettings?.scale
						?.minReplicas ?? 1,
	...
	public async updateEnvironmentVariables(environmentVariables: {
		[p: string]: string;
	}): Promise<void> {
		ext.outputChannel.appendLog(
	...
	public getJvmOptions(): string {
		const enterpriseOptionsStr: string | undefined =
			this.properties?.deploymentSettings?.environmentVariables
				?.JAVA_OPTS;
	...
	public getScaleSettings(): IScaleSettings {
		const settings: DeploymentSettings | undefined =
			this.properties?.deploymentSettings;
		...
		const capacity: number = this.app.service.isConsumptionTier()
			? // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
			  (this.properties?.deploymentSettings?.scale
	...
	public async getDebuggingConfig(): Promise<RemoteDebugging | undefined> {
		if (this.app.service.isConsumptionTier()) {
			ext.outputChannel.appendLog(
				`[Deployment] remote debugging is not supported for apps of consumption plan.`
			);
			return undefined;
		}
