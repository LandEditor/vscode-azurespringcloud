...
export class ValidateRuntimeStep extends AzureWizardExecuteStep<IAppDeploymentWizardContext> {
	...
	public async execute(
		...
		if (parseInt(versionStr) < artifactVersion - 44) {
			throw new Error(`The bytecode version of artifact (${
				this.artifactPath
			}) is "${artifactVersion} (Java ${artifactVersion - 44})",
            which is incompatible with the runtime "Java ${versionStr}" of the target app (${
	...
	public shouldExecute(_context: IAppDeploymentWizardContext): boolean {
		return (
			!this.deployment.app.service.isEnterpriseTier() &&
			this.deployment.runtimeVersion
				?.split(/[\s\_]/)?.[0]
				.toLowerCase() === "java"
		);
	}
	...
	private async getClassByteCodeVersion(
		...
		const manifest: { [key: string]: string } =
			await this.readManifest(manifestStream);
		(manifestStream as ReadStream).destroy();

		// Main-Class
		const mainClass: string | undefined = manifest["Main-Class"]
			?.replace(/\./g, "/")
			?.concat(".class");
		if (!mainClass) {
			throw new Error(
